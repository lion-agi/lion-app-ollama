import traceback
from pydantic import BaseModel, Field, ConfigDict, model_validator, field_serializer, SerializeAsAny
...
from lion_ollama.api_endpoints.api_request import OllamaRequest
from lion_ollama.api_endpoints.data_models import OllamaEndpointRequestBody
from lion_ollama.api_endpoints.completion.request_body import (
    OllamaCompletionRequestBody,
)
from lion_ollama.api_endpoints.chat_completion.request_body import (
    OllamaChatCompletionRequestBody,
)
from lion_ollama.api_endpoints.embedding.request_body import OllamaEmbeddingRequestBody
from lion_ollama.api_endpoints.match_response import match_response
...
class OllamaModel(BaseModel):
    ...
    async def invoke(
        ...
        if ((isinstance(request_body, OllamaCompletionRequestBody) and not getattr(request_body, "prompt")) or
                (isinstance(request_body, OllamaChatCompletionRequestBody) and not getattr(request_body, "messages"))):
    ...
    async def stream(self, request_body: OllamaEndpointRequestBody, output_file=None, parse_response=True):
        response_list = []
    ...
    def get_input_token_len(self, request_body: OllamaEndpointRequestBody):
        ...
        if isinstance(request_body, OllamaCompletionRequestBody):
            ...
            messages_text = " ".join(messages_text) if isinstance(messages_text, list) else messages_text
            text_tokens = self.text_token_calculator.calculate(messages_text)
            return text_tokens
        else:
            return 0
